...
Each package keeps `main.ts`, `README.md`, `AGENTS.md`, `CHANGELOG.md`, and
...
entries such as `#shared/citoid`, `#shared/html`, and `#shared/wikitext`.

The package README uses `Run`, `Features`, `Development`, `Architecture`, and
`License` sections and includes its real dependency tree. The active changelog
...
Completed gadget changes are recorded in each package's `CHANGELOG.md`, linked
from its README.

## License

Each deployable gadget is released under [CC BY-SA 4.0][5]. The repository
license notice is in [LICENSE][6]. Although package metadata may identify an
