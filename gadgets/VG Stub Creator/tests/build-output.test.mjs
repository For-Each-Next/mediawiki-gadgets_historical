...
test("minified output is wrapped in MediaWiki nowiki comments", async () => {
    const source = await readFile("dist/vg_stub_creator.min.js", "utf8");
...
test("userscript output includes Tampermonkey metadata and bundled code", async () => {
    const source = await readFile("dist/vg_stub_creator.user.js", "utf8");

    assert.equal(source.startsWith("// ==UserScript==\n"), true);
    assert.match(source, /^\/\/ @name {9}VG Stub Creator$/mu);
    assert.match(source, /^\/\/ @version {6}0\.4\.17$/mu);
    assert.match(
        source,
...
test("stylesheet output includes dialog styles", async () => {
    const source = await readFile("dist/vg_stub_creator.css", "utf8");

    assert.match(source, /\.vg-stub-creator-dialog\.cdx-dialog/u);
    assert.match(source, /\.vg-stub-creator-preview-layout/u);
});
...
async function getLoaderCallCount(action, articleId = 1, dbName = "") {
    const source = await readFile("dist/vg_stub_creator.js", "utf8");
    let calls = 0;
    const sandbox = {
        window: {
            location: {
