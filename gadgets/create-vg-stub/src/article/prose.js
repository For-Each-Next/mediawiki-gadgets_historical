...
import { buildAggScoresText } from "../fragments/agg-scores.js";
import { buildLeadNameText } from "../fragments/lead-name.js";
import { countProseSinographs } from "../prose-count.js";

/**
 * Builds every prose fragment that combines independent data parts.
 *
 * @param {object} parts - Article part outputs keyed by part.
 * @param {object} sourceTags - Source reference tags keyed by field.
 * @returns {object} Composed article prose fragments.
 */
export function buildArticleProse(parts, sourceTags) {
    const names = parts.names.metadata;
    const leadValues = {
    ...
    const companies = buildCompanyProse(parts.companies, {
    ...
    const scores = buildScoreProse(parts.scores, sourceTags);
    const additional = buildAdditionalProse(
        parts.additionalProse,
