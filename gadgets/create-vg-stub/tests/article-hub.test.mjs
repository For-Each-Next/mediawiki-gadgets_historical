/**
 * Tests the registered article data hub.
 */
...
test("article parts emit one shared metadata contract", () => {
    ...
    assert.deepEqual(
        Object.keys(data.parts),
        ARTICLE_PARTS.map(({ key }) => key),
    );
    Object.values(data.parts).forEach((part) => {
        assert.deepEqual(Object.keys(part).sort(), contract);
    ...
    assert.equal(data.parts.scores.values[0].normalizedText, "PS5:85");
    assert.equal(data.parts.scores.values[0].displayText, "PS5:85");
    assert.equal(data.parts.scores.values[0].linkTarget, "");
    assert.equal(data.parts.scores.values[0].wikitext, "PS5:85");
    assert.deepEqual(data.parts.scores.sourceUrls, [
