...
test("buildEditSummary prefers Wikidata title metadata", () => {
    assert.equal(
        ...
        `create '«サンプル»' (2026), 59 equivalent sinographs; see '[[:w:en:Example Game]]' and '[[:d:Q123]]' ${EDIT_SUMMARY_SUFFIX}`,
    );
});
...
test("buildEditSummary falls back to enwiki title metadata", () => {
    assert.equal(
        ...
        `create '«サンプル»' (2026), 59 equivalent sinographs; see '[[:w:en:Example Game]]' ${EDIT_SUMMARY_SUFFIX}`,
    );
});
...
test("buildEditSummary renders plain title without enwiki metadata", () => {
    assert.equal(
        ...
        `create '«サンプル»' (2026), 59 equivalent sinographs ${EDIT_SUMMARY_SUFFIX}`,
    );
});
