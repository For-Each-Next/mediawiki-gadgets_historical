...
import {
    ...
    saveTalkAssessment,
} from "../../src/vg-page-assessor/api.ts";
...
test("fetches creation times in a same-namespace batch", async () => {
    ...
    assert.equal(
        times.get("Redirect").toISOString(),
        "2026-06-15T00:00:00.000Z",
    );
    assert.equal(
        times.get("Target").toISOString(),
        "2026-06-15T00:00:00.000Z",
    );
    ...
    assert.deepEqual(
        calls
            .filter((call) => call.rvprop === "timestamp")
            .map((call) => call.titles),
        ["Target|Plain"],
    );
});
...
test("uses localStorage cached creation datetimes", async () => {
    ...
    try {
        ...
        if (originalLocalStorage === undefined) {
            ...
            testGlobal.localStorage = originalLocalStorage;
        }
    }
});
