...
import { minifiedOutput } from "../../build.config.ts";
...
test("minified output is wrapped in MediaWiki nowiki comments", async () => {
    ...
    assert.match(source, /^\/\/ @version {6}0\.4\.18$/mu);
    assert.match(
        source,
        /^\/\/ @description {2}Quickly create a video game stub on the Chinese Wikipedia\.$/mu,
    );
    assert.match(
        source,
        new RegExp(
...
test("userscript output includes Tampermonkey metadata and bundled code", async () => {
    const source = await readFile(
        "../../dist/vg-stub-creator/vg_stub_creator.user.js",
        "utf8",
    );
