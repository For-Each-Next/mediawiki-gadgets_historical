...
test("year alias uses two digits", async () => {
    ...
    assert.ok(
        text.startsWith(`${EXAMPLE_HEADER}\n\n《'''Example'''》是2024年`),
    );
});
...
test("series renders after platform text", async () => {
    ...
    assert.equal(
        text.includes("作品对应PC平台，属于「《Example》系列」。"),
        true,
    );
});
...
test("starred series renders as derivative work text", async () => {
    ...
    assert.equal(
        linkedText.includes("作品属于「《[[Example]]》衍生作品」。"),
        true,
    );
...
test("series suffix is trimmed before rendering", async () => {
    ...
    assert.equal(
        text.includes("作品对应PC平台，属于「《最終幻想》系列」。"),
        true,
    );
...
test("series source reference renders after series text", async () => {
    ...
    assert.equal(
        text.includes(
            '作品对应PC平台，属于「《Example》系列」<ref name=":1" />。',
        ),
        true,
    );
    assert.equal(
...
async function buildStubText(values) {
    ...
    return vm.runInContext(
        `vgStubCreator.buildStubText(
      vgStubCreator.createArticleParams(${JSON.stringify(values)})
    )`,
        sandbox,
    );
}
...
async function createSandbox() {
    const source = await readFile(
        "../../dist/vg-stub-creator/vg_stub_creator.min.js",
        "utf8",
    );
    const sandbox = {
        $() {
