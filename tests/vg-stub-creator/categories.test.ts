...
} from "../../src/vg-stub-creator/handlers/categories.ts";
...
} from "../../src/vg-stub-creator/wikitext/categories.ts";
...
function emptyParams(options: Record<string, any> = {}) {
    return {
        companyMetadata: {
            categories: [],
            stubTags: [],
...
function paramsFromForm(form, options: Record<string, any> = {}) {
    const params: any = createArticleData({
        categoryRows: [],
...
function createRedirectCategoryFetcher(redirects: Record<string, any>) {
    return async (url) => {
        const titles = new URL(url, "https://zh.wikipedia.org").searchParams
            .get("titles")
            .split("|")
...
function createOrdinaryRedirectCategoryFetcher(
