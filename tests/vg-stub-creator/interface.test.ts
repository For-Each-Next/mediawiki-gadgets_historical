/**
 * Tests vg-stub-creator dialog behavior.
 */
...
import {
    ...
    createPreSaveGroups,
} from "../../src/vg-stub-creator/interface/form/index.ts";
import fieldData from "../../src/vg-stub-creator/field-data.ts";

const testGlobal = globalThis as any;
const originalWindow = testGlobal.window;
const originalMw = testGlobal.mw;
const originalCitationTemplate = fieldData["citation-template"];
beforeEach(() => {
    testGlobal.window = {};
});
...
afterEach(() => {
    ...
    testGlobal.mw = originalMw;
    fieldData["citation-template"] = originalCitationTemplate;
});
...
test("history JSON can be copied, edited, and imported", async () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-history-json-text"),
        true,
    );
...
test("References tab manages editable citation parameters", async () => {
    ...
    const component = createDialogComponent(
        createVueStub(),
        createOptionsStub({
            async onPrepareCitations(_form, options: any = {}) {
    ...
    assert.equal((citationTableColumns[0] as any).width, undefined);
...
test("additional prose uses a textarea and source URL field", () => {
    ...
    assert.equal(
        component.template.includes(
            '<cdx-text-area class="vg-stub-creator-article-field-text" rows="1"',
        ),
        true,
    );
    assert.deepEqual(
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-prose-length"),
        false,
    );
    assert.equal(
        component.template.includes(
...
test("metadata source fields render in a table", () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-metadata-table"),
        true,
    );
    assert.deepEqual(
    ...
    assert.equal(
        component.template.includes(
            '<cdx-table class="vg-stub-creator-metadata-table"',
        ),
        true,
    );
    assert.equal(
...
test("foreign title fields share one fieldset label", () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-fieldset-fields"),
        true,
    );
    assert.equal(
...
test("review exposes editable navboxes and subtle prose length", async () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-review-row-converted"),
        false,
    );
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-category-actions"),
        false,
    );
    assert.equal(component.template.includes('v-model="row.enabled"'), true);
    assert.equal(component.template.includes('v-model="row.enabled"'), true);
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-destructive-action"),
        true,
    );
    assert.equal(
...
test("original title lookup is separate from the Steam helper", () => {
    ...
    assert.equal(
        component.template.includes('<ul class="vg-stub-creator-name-search"'),
        true,
    );
    assert.equal(
...
test("category helper stages missing category rows for final submission", async () => {
    ...
    assert.equal(
        component.template.indexOf("English Wikipedia category") <
            component.template.indexOf(
                "vg-stub-creator-company-category-text",
            ),
        true,
    );

    const genericRow: any = {
        category: "动作游戏",
        status: "Not exists",
    };
...
test("Steam helper stages official localized name choices", async () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-steam-helper"),
        true,
    );
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-steam-links"),
        true,
    );
...
test("localized name rows separate official and region checkboxes", () => {
    ...
    assert.equal(
        component.template.includes(
            '<span aria-hidden="true" class="vg-stub-creator-name-market-separator"></span>',
        ),
        true,
    );
...
test("field preview helper receives live form and preview key", () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-wikitext-preview"),
        false,
    );
    assert.equal(
...
test("submit opens preview without changing tabs", async () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-dialog-mask"),
        true,
    );
...
test("submit confirms changed page name before previewing", async () => {
    ...
    assert.equal(state.previewOpen.value, false);
    assert.equal(previewCount, 2);
    assert.equal(component.template.includes("Preview without moving"), true);
    assert.equal(
        component.template.includes(
...
test("native submit bridge opens category and pre-save review", async () => {
    ...
    await window.vgStubCreatorDialog.submit();
...
test("submit opens editable source and parsed preview first", async () => {
    ...
    assert.equal(
        component.template.includes("vg-stub-creator-preview-text"),
        true,
    );
    assert.equal(
        component.template.includes("vg-stub-creator-preview-dialog"),
        true,
    );
    assert.equal(
        component.template.includes("vg-stub-creator-preview-layout"),
        true,
    );
...
test("enwiki lookup fills wikidata and blank English title", async () => {
    ...
    const openedLinks = [];
    const focusedLinks = [];
    testGlobal.window.open = (url, target) => {
...
function createOptionsStub(options: any = {}) {
    return {
        defaultName: "Example",
        getProseSinographs() {
            return 0;
