...
import {
    ...
    movePage,
    runSelectedActions,
} from "../../src/vg-stub-creator/editing/pre-save.ts";
...
test("runSelectedActions can run registration before Wikidata edits", async () => {
    const calls = [];
    const api = {
        async get() {
            return {
    ...
    await runSelectedActions(
        ...
        {
            api,
            async onBeforeWikidataActions(result) {
                calls.push(["registration", result.title]);
            },
            title: "Target",
            wikidataApi,
        },
...
/**
 * Creates a MediaWiki API stub and records its write calls.
 *
 * @param calls - Mutable list of recorded API calls.
 * @param getResponse - Response returned from API reads.
 * @returns MediaWiki API stub used by pre-save tests.
 */
function createApiStub(
    calls: Array<Record<string, any>>,
    getResponse: Record<string, any> = { query: { pages: {} } },
): Record<string, any> {
    return {
        async get(params) {
            calls.push(["get", params]);
