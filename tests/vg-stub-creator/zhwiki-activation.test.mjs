...
} from "../../src/vg-stub-creator/src/sources/zhwiki-activation.js";
...
test("buildZhwikiCreationUrl carries the English title to zhwiki edit view", () => {
    ...
    assert.equal(url.searchParams.get("vg-stub-creator"), "1");
    assert.equal(
        url.searchParams.get("vg-stub-creator-enwiki-title"),
        "Example Game (video game)",
    );
});
...
test("readZhwikiActivationForm reads the incoming enwiki title", () => {
    assert.deepEqual(
        readZhwikiActivationForm(
            "?action=edit&vg-stub-creator=1&" +
                "vg-stub-creator-enwiki-title=Example+Game",
        ),
        {
